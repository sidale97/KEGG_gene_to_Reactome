...
path_towards_tool_directory="$(dirname $0)"
#echo moving PWD from $PWD
#echo to $path_towards_tool_directory
cd "$path_towards_tool_directory"

if ! test -e 'load_thread.sh' && ! test -e 'cross_references_translate_thread.sh' ;
then 
  echo '"load_thread.sh" and "cross_references_translated.sh" not found... probably you are sourcing "main.sh" and your working directory is not "'"$(realpath . )"'". Please change your working directory to such path, and invoke "main.sh" again, or avoid sourcing.' ;
  return 1;
fi ;
...
do
  ...
  '-j')
    ...
    is_natural_number="$(echo "$n" | grep '^[0-9][0-9]*$' )" ;
...
do eval "ensembl_to_${x}=ftp.ensembl.org/pub/current_tsv/homo_sapiens/Homo_sapiens.GRCh38.103.${x}.tsv.gz"
  ...
  if_exists_remove "$parallel_fifo" "$parallel_fifo"2 ;
...
load_missing_new_cross_references () {
  ...
  prepare_neo4j ;
  if_exists_remove "$parallel_fifo"2
  mkfifo  "$parallel_fifo"2;

  ls "$cross_references_directory_diff" >"$parallel_fifo"2 &

  assign_to_threads load_missing_new_cross_references_thread <"$parallel_fifo"2 ;
  assign_to_threads_exit_status="$?" ;
  rm -f "$parallel_fifo"2 ;
...
auto (){
echo
echo AUTO EXECUTING OPERATION: "login"
if ! login "$@" ; then return 1 ; fi ; #yes, this $@ includes the "auto" sub command, bc "login" also includes its sub command in the input
for command in  "download new genes" "count missing new genes" "load missing new genes" "download new cross references" "count missing new cross references" "load missing new cross references" ;
do
  if test "$number_of_parallel_jobs" = '100%' ;
  then  option_j=0 ;
  else option_j="$number_of_parallel_jobs"
  fi ;
  
  echo
  echo AUTO EXECUTING OPERATION: "\"$command\""
  if test "$option_f_set" ;
  then ./main.sh -f -j "$option_j" $command
  else ./main.sh    -j "$option_j" $command
  fi ;
